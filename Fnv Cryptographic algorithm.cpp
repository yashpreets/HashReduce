...
/* Self-check */
 ...
 string self_check(string s) {
	...
	for(int i = 0;i < l;i++)
		ptr[i] = s[i];
	md5_hash((uint8_t*)ptr, l, hash);
	ans = atob(hash[0])+atob(hash[1])+atob(hash[2])+atob(hash[3]);
	return ans;
}
